...
class SSPMiniGridMissionWrapper(SSPMiniGridViewWrapper):
    ...
    def _encode_words(self, words):
        O_w = self.sp_space.name_to_vector['I']
        for w in words:
            w = w.strip()
            if w in self.color_map.keys():
                O_w += self.sp_space.bind(O_w, self.sp_space.name_to_vector[w])
    ...
    def _encode_submission(self, mission):
        O = np.zeros(self.shape_out)
        for instruction in self.go_to_patterns:
            if re.match(".*" + instruction + ".*", mission):
                words = mission.partition(instruction)[2].split(" ")
                O += self.sp_space.bind(self.sp_space.name_to_vector['GO_TO'] , self._encode_words(words))

        for instruction in self.pick_up_patterns:
            if re.match(".*" + instruction + ".*", mission):
                words = mission.partition(instruction)[2].split(" ")
                O += self.sp_space.bind(self.sp_space.name_to_vector['PICK_UP'], self._encode_words(words))
        ...
        for instruction in self.put_somewhere_patterns:
            if re.match(instruction, mission):
                ...
                O += self.sp_space.bind(self.sp_space.name_to_vector['PUT'], self._encode_words(put_what))
                O += self.sp_space.bind(self.sp_space.name_to_vector['NEXT_TO'], self._encode_words(put_where))

        for instruction in self.open_something_patterns:
            if re.match(".*" + instruction + ".*", mission):
                words = mission.partition(instruction)[2].split(" ")
                O += self.sp_space.bind(self.sp_space.name_to_vector['OPEN'], self._encode_words(words))
        ...
        if re.match("GET THE .* KEY FROM THE .* ROOM, UNLOCK THE .* DOOR AND GO TO THE GOAL", mission): #for MiniGrid-LockedRoom-v0
            ...
            O += self.sp_space.bind(self.sp_space.name_to_vector['GO_TO'],
                                    self.sp_space.name_to_vector['DOOR'],
    ...
    def _encode_mission(self, mission):
        ...
        for m in submissions:
            mission_sp += self._encode_mission(m)
        return mission_sp

    def observation(self, obs):
        img = obs['image']
        
        agt_ssp = self._encode_agent_pos()
        has_sp = self._encode_carry()
        view_sp = self._encode_view(img)

        mission_sp = self._encode_mission(obs['mission'])

        vsa_encoding = np.hstack([agt_ssp + has_sp + view_sp, mission_sp]).flatten()
...
class PrepMiniGridMissionWrapper(SSPMiniGridViewWrapper):
    def __init__(
        self,
        env: gym.Env,
        shape_out = None,
        **kwargs
        ...
        super().__init__(env, shape_out, **kwargs)
    ...
    def observation(self, obs):
        ...
        return {
            'mission': obs['mission'],
            'image': np.hstack([agt_pt, np.array(obj_sps).flatten(), has_vector, mission_vector])
        }
